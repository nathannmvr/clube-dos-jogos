...
export default function EditGameForm({ game }: { game: Game }) {
  ...
  const handleSelectGame = (g: RawgResult) => {
    setTitle(g.name);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (response.ok) {
      ...
      setTimeout(() => { router.push('/admin/manage-games'); router.refresh(); }, 1500);
    } else {
  ...
  const previewCover = manualCover.trim() || coverUrl;

  return (
    <form onSubmit={handleSubmit} style={{ fontFamily: "'VT323', monospace" }}>
      {/* Title with RAWG search */}
      <div ref={containerRef} style={{ marginBottom: '24px', position: 'relative' }}>
        ...
        <input type="text" value={title} onChange={e => setTitle(e.target.value)} required className="retro-input" />
        {(rawgResults.length > 0 || searching) && (
          <div style={{
            position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50,
            background: '#08081a', border: '2px solid #39ff14',
            boxShadow: '0 8px 24px rgba(57,255,20,0.2)', maxHeight: '300px', overflowY: 'auto',
          }}>
            {/* Dismiss button */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 12px', borderBottom: '1px solid #1a1a3a', background: '#0d0d1a' }}>
              <span style={{ fontFamily: "'VT323'", fontSize: '15px', color: '#6060a0' }}>Selecione ou feche e salve diretamente</span>
            ...
            {searching && <p style={{ padding: '12px', color: '#6060a0', fontFamily: "'Press Start 2P'", fontSize: '8px' }}>BUSCANDO<span className="blink">...</span></p>}
            {rawgResults.map(g => (
              <button key={g.id} type="button" onClick={() => handleSelectGame(g)} style={{
                width: '100%', display: 'flex', alignItems: 'center', gap: '12px',
                padding: '10px 12px', background: 'transparent', borderBottom: '1px solid #1a1a3a', cursor: 'pointer',
              }}
                onMouseEnter={e => (e.currentTarget.style.background = '#12122a')}
                onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
              >
                {g.coverUrl ? (
                  <Image src={g.coverUrl} alt={g.name} width={48} height={32} style={{ objectFit: 'cover', border: '1px solid #2a2a5a' }} />
                ) : (
                  <div style={{ width: 48, height: 32, background: '#1a1a3a', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6060a0', fontSize: '10px' }}>?</div>
                )}
                <span style={{ color: '#00f5ff', fontSize: '18px' }}>{g.name}</span>
              </button>
            ))}
          </div>
        )}
      ...
      {previewCover && (
        <div style={{ marginBottom: '16px' }}>
          <p style={{ fontFamily: "'Press Start 2P'", fontSize: '8px', color: '#39ff14', marginBottom: '8px' }}>✓ CAPA ATUAL:</p>
          <div className="pixel-border-green" style={{ display: 'inline-block' }}>
            <Image src={previewCover} alt="Cover" width={200} height={130} style={{ objectFit: 'cover', display: 'block' }} onError={() => {}} />
          </div>
        </div>
      )}
